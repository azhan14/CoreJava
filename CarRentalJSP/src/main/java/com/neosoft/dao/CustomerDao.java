...
import com.neosoft.beans.CustomerBean;
...
public class CustomerDao {
	...
	public static int rentCar(String carno, int custid,String custname, int carprice) {
		int status = 0;
		try {
			Connection con = DB.getCon();
			String insertQuery = "INSERT INTO RENTCAR2(CARNO,CUSTID,CUSTNAME,CARPRICE) VALUES(?,?,?,?)";
			PreparedStatement pst = con.prepareStatement(insertQuery);
			...
			status = pst.executeUpdate();
			if(status > 0) {
				String updateQuery = "UPDATE CAR2 SET CARSTATUS = 'NO' WHERE CARNO = ?";
